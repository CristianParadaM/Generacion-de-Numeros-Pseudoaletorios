...
public class DistribucionUniforme {
	...
	public double[] generatePseudorandomNumbers(){
		...
		for (int i = 0; i < numbers.length; i++) {
			numbers[i] = min + (max - min) * this.ri[i];
		}
		return numbers;
	}
	...
	public ArrayList<Double[]> generateIntervals(){
		...
		for (int i = 0; i < NUMBER_OF_INTERVALS; i++) {
			Double[] aux = new Double[] {inicio, Double.parseDouble(new BigDecimal(String.valueOf(inicio+((max-min)/(double)NUMBER_OF_INTERVALS))).setScale(5, RoundingMode.FLOOR).toString())};
			intervals.add(aux);
			inicio = aux[1];
		}
		return intervals;
	...
	public static void main(String[] args) {
		new DistribucionUniforme(10, 200, new Minimos_Cuadrados(3456, true).getRi()).show();
	}
